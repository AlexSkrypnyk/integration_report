...
This is a Integration Report template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `integration_report.*` - Template extension files
- `IntegrationReportService.php` - Main service class template
...
1. Run `php init.php` to customize template for Integration Report
2. Build environment: `make build` or `ahoy build`
3. Develop Integration Report code in `src/`
...
- SQLite database created in `/tmp/site_integration_report.sqlite`
